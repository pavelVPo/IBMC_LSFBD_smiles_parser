...
end_allowed <- data_inProgress |> filter(
								  ...
								  # exclude the forbidden repeated in-bracket pairs
								  filter( left_charClass != "r_isotope" & left_charClass != "s_isotope" & 
								  		  left_charClass != "r_chiral" & left_charClass != "s_chiral_m" &
								  		  left_charClass != "e_hydro" & left_charClass != "s_hydro" &
								  		  left_charClass != "e_charge" & left_charClass != "s_charge" &
								  		  left_charClass != "e_charge" & left_charClass != "s_charge_m" &
								  		  left_charClass != "e_chiral" & left_charClass != "s_chiral" &
								  		  left_charClass != "e_atom_bal" & left_charClass != "s_atom_bal" &
								  		  left_charClass != "e_atom_bar" & left_charClass != "s_atom_bar"

								  )
# Process further only pairs, which have the correct order
data_inProgress <- bind_rows(start_allowed, inter_allowed, end_allowed)
...
bracket_allowed   <- data_inProgress |> filter(
											...
											( right_charClass == "s_bracket" & left_charClass %in% c("e_bracket", "l_ct", "r_ct", "w_anything", "w_aromatic_bond", "w_atom_bal", "w_atom_oal", "w_atom_oar", "w_bm_ibi", "w_bm_iri", "w_bm_tbi", "w_bm_tri", "w_no_bond", "w_quadruple_bond", "w_single_bond", "w_triple_bond")) |
											...
											(left_charClass == "e_bracket"   & right_charClass %in% c("s_bracket", "l_ct", "r_ct", "w_anything", "w_aromatic_bond", "w_atom_bal", "w_atom_oal", "w_atom_oar", "w_bm_ibi", "w_bm_iri", "w_bm_tbi", "w_bm_tri", "w_no_bond", "w_quadruple_bond", "w_single_bond", "w_triple_bond"))
										)
...
# Export pairs
write_tsv(data, "C:/.../theory_pairs_charClasses_rules.tsv")