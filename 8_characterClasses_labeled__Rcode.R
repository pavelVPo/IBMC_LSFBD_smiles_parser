...
whole_allowed <- data_inProgress |> filter( (left_charClass %in% c("w_anything", "w_aromatic_bond", "w_atom_bal", "w_atom_bar", "w_atom_oal", "w_atom_oar",
...
data_inProgress <- data_inProgress |> anti_join(whole_allowed, by = c("left_charClass", "right_charClass"))
...
start_allowed <- data_inProgress |> filter(
								...
								# left start, multiple
								(left_charClass == "s_chiral_m" & right_charClass == "m_chiral") |
								# left start, 4-character
								(left_charClass %in% c("s_bm_ire_4", "s_bm_tre_4") & right_charClass == str_replace(left_charClass, "^s_", "n_") |> str_replace("_4", "")) |
								# left start, rest
								(left_charClass %in% c("s_isotope", "s_charge_m", "s_class", "s_bm_iri", "s_bm_tri") & right_charClass == str_replace(left_charClass, "^s_", "r_") |> str_replace("_m", ""))
							)
...
end_allowed <- data_inProgress |> filter(
										# left end
										(left_charClass %in% c("e_atom_bal", "e_atom_bar", "e_atom_oal", "e_bm_ibe", "e_bm_ire", "e_bm_tbe", "e_bm_tre", "e_charge", "e_chiral", "e_hydro",
											"r_bm_ire", "r_bm_iri", "r_bm_tre", "r_bm_tri", "r_charge", "r_chiral", "r_class", "r_isotope") &
