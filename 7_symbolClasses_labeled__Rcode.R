...
pairs_symbTypes_not <- read_tsv("C:/.../theory_pairs_symbType.tsv") |>
                            filter(allowed == "no")
...
write_tsv(pairs_symbClass, "C:/.../theory_pairs_symbClass.tsv")

###
# Isotope symbols could only be paired with the bracket on the left and bracket atom on the right, also isotope_m could be paired with itself on both sides.
...
# Features symbols, besides isotope, could only be paired with the other feature symbols or bracket symbols on the right and with the other feature symbols or bracket atom symbols on the left.
...
fs_allowed <- pairs_labeled |> filter(
                               ...
                               select(left_symbClass, right_symbClass) |> distinct()
# Bracket atoms cannot be paired with the symbols contained outside the brackets and they can only be paired with the isotope symbols if those symbols are on the left and they can only be paired with the other bracket atom symbols on condition that those symbols has the same length, which is greater than 1.
ba_allowed <- pairs_labeled |> filter(
                                ...
                                select(left_symbClass, right_symbClass) |> distinct()
# Organic atom symbols can not be paired with the symbols contained inside the brackets.
oa_allowed <- pairs_labeled |> filter(
                                ...
                                select(left_symbClass, right_symbClass) |> distinct()
# Bond symbols and bond modifying symbols can not be paired with the symbols contained inside the brackets.
...
pairs_unknown <- pairs_labeled |> filter(
                                        !str_detect(left_symbClass,  "bm.*|.*_bond|atom_b.*|isotope.*|chiral.*|hydro.*|charge.*|class.*|chiral.*") &
                                        !str_detect(right_symbClass, "bm.*|.*_bond|atom_b.*|isotope.*|chiral.*|hydro.*|charge.*|class.*|chiral.*")
                                ) |>
                                select(left_symbClass, right_symbClass) |> distinct()
# Gather labeled pairs
pairs_rulesApplied <- bind_rows(isotope_allowed, fs_allowed, ba_allowed, oa_allowed, bbm_allowed, pairs_unknown)
# Export these prefiltered results
write_tsv(pairs_rulesApplied, "C:/.../theory_pairs_symbClass_rules.tsv")